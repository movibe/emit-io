...
class MockEmitIoStrategy
  implements
    LoggerStrategyType<string, string, User, BeginCheckoutEvent, PurchaseLogEvent, EVENT_TAGS>
{
  init = vi.fn()
  log = vi.fn()
  event = vi.fn()
...
describe('EmitIoStrategy — Legacy API (backward compat)', () => {
  ...
  beforeEach(() => {
    ...
    logger = new EmitIoStrategy([{ class: mockStrategy, enabled: true }])
  })
