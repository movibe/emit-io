...
import { createMockTransport, createMockProvider, createTestLogger } from '../test-utils.js'
...
describe('createMockProvider', () => {
  ...
  test('events with properties', () => {
    const provider = createMockProvider()
    const logger = new LoggerStrategy({ providers: [provider], emitAppOpenOnInit: false })
    logger.event('add-to-cart', { product_id: 'p1', quantity: 2 })
  ...
  test('screens array populated by logScreen', () => {
    const provider = createMockProvider()
    const logger = new LoggerStrategy({ providers: [provider], emitAppOpenOnInit: false })
    logger.logScreen('HomeScreen', { from: 'nav' })
...
describe('createTestLogger', () => {
  ...
  test('provider events accessible', () => {
    const { logger, provider } = createTestLogger()
  ...
  test('clear() works independently on transport and provider', () => {
    const { logger, transport, provider } = createTestLogger()
    logger.info('msg')
    logger.event('user-login', { method: 'email' })
    transport.clear()
    provider.clear()
