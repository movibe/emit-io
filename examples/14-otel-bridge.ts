...
emit.event('checkout-started', { sku: 'item-abc', amount: 99 })

// captureError -> OTel span with error attributes + log with severity ERROR
emit.captureError('Payment', 'charge-failed', true, new Error('card declined'))

// Log-level calls -> OTel log records with matching severity
emit.info('Order processed', { orderId: 'ord-1' })
emit.error('DB timeout', { table: 'orders', ms: 5000 })
