import { EmitIoStrategy, ConsoleTransport, runWithContext, LogLevelEnum } from 'emit-io-core'
...
async function processRequest(traceId: string, requestId: string) {
  await runWithContext({ traceId, requestId }, async () => {
    emit.info('request started')    // context: { traceId, requestId }

    await fetchUser('usr-1')          // context flows into nested calls automatically
    await writeAuditLog()

    emit.info('request complete')   // context: { traceId, requestId }
  })
}

async function fetchUser(id: string) {
  // No manual context passing needed — ALS provides it
  emit.debug('fetching user', { id })
}

async function writeAuditLog() {
  emit.debug('writing audit log')
}
