...
| What changed | v2 | v3 |
|---|---|---|
| Analytics error call | `emit.error(feature, name, critical, err, extra)` | `emit.captureError(feature, name, critical, err, extra)` |
| Analytics info call | `emit.info(feature, name, properties)` | `emit.logFeature(feature, name, properties)` |
| Typed event registry | `declare global { interface EVENT_TAGS { ... } }` | `declare module 'emit-io-core' { interface EventRegistry { ... } }` |
| Config format | `LoggerStrategyConstructor[]` | `EmitIoStrategyConfig` with `AnalyticsProvider` instances |

Log-level calls `emit.error('message')` and `emit.info('message')` are **unchanged**.
...
```bash
npm install emit-io-core@^1.0.0
```

Or with bun:

```bash
bun add emit-io-core@^1.0.0
```
...
| Find | Replace |
|---|---|
| `emit.error('Feature', 'name',` | `emit.captureError('Feature', 'name',` |
| `emit.info('Feature', 'name',` | `emit.logFeature('Feature', 'name',` |
...
emit.warn('Retry attempt', { count: 3 })
emit.debug('Cache miss', { key: 'user:42' })
```
...
```typescript
import { EmitIoStrategy, LoggerStrategyType } from 'emit-io-core'
...
const emit = new EmitIoStrategy({ providers: [myProvider] })
```
...
```typescript
const reqLog = emit.child({ requestId: 'req-123' })
...
```typescript
import { JSONTransport, HTTPTransport, DevToolsTransport } from 'emit-io-core'
```
...
| Item | Status |
|---|---|
| `LoggerStrategyConstructor[]` config | Works, emits `console.warn`. Migrate to `EmitIoStrategyConfig`. |
...
| Item | Status |
|---|---|
| `emit.error(feature, name, critical, err)` overload | **Removed**. Use `emit.captureError(...)`. |
| `emit.info(feature, name, properties)` overload | **Removed**. Use `emit.logFeature(...)`. |
