...
|---|---|
| `@emit/core` | `JSONTransport` with `write` piped to `/dev/null`, `emitAppOpenOnInit: false` |
...
|---|---|---|---|---|---|
| `@emit/core` | 1,202,455 | 976.10 | 833.00 | ±3.73% | 1,024,490 |
...
| Library | ops/sec (avg) | Latency avg (ns) | Latency med (ns) | Margin | Samples |
|---|---|---|---|---|---|
| `@emit/core` | 981,834 | 1,872.6 | 959.00 | ±40.72% | 604,457 |
...
| Library | ops/sec (avg) | Latency avg (ns) | Latency med (ns) | Margin | Samples |
|---|---|---|---|---|---|
| `@emit/core` | 1,099,641 | 1,837.8 | 875.00 | ±9.64% | 545,564 |
...
Pino is the throughput leader across all three scenarios, running roughly 2× faster than `@emit/core` at median. This is expected: pino is purpose-built for raw log throughput with minimal per-call overhead. `@emit/core` beats winston in the **context scenario** (median ~1,043K vs ~889K ops/s, ~17% ahead) but is roughly on par with winston in the simple and child scenarios when judged by median rather than mean (which is skewed by outliers in both). The main takeaway: `@emit/core` runs comfortably in the 1M ops/s range and is never slower than winston, while offering features neither pino nor winston provide out of the box — circuit breaker, per-transport rate limiting, typed analytics events, and consent management.
